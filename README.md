...
* Open the nodes are started, go to the terminal of Party A (not the notary!)
  and run the following command to issue 99 tokens to Party B:
    ...
    `flow start AnnexureIssueFlowInitiator owner: PartyC, amount: 99`
    `flow list`
    `flow start AnnexureIssueFlowInitiator`
    `flow start AnnexureIssueFlowInitiator owner: PartyC, id: 1, policyId: 2, siteName: Google, status: draft, creationDate: 12-Oct-2019`
